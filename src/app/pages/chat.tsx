...
export const Chat = ({ userId }: Props) => {
  ...
  const [isTypingIndicatorDisplayed, setIsTypingIndicatorDisplayed] =
    useState(false);
  ...
  const submit = async (message: string) => {
    ...
    fetch("https://chat-vitiligo.onrender.com/question.ask", {
      method: "POST",
      headers: {
  ...
  return (
    <div className="flex flex-col touch-manipulation h-[100vh] max-w-[780px] m-0 mx-aut bg-pastel1 m-0 mx-auto">
      <Header onLogoClick={() => setIsMenuDrawerVisible(true)} className={classNames("sticky top-0 left-0 w-full")} />
      <div
      ...
      {isMenuDrawerVisible && (
        <MenuDrawer onClose={() => setIsMenuDrawerVisible(false)} />
      )}
      <UserInput
        className={classNames(
